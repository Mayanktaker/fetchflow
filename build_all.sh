...
zip -r "$OUT_DIR/xdm-chrome-extension-9.0.0.zip" .
cd ../../..
...
if [ -n "$MOZ_API_KEY" ] && [ -n "$MOZ_API_SECRET" ]; then
    ...
    cp web-ext-artifacts/*.xpi "$OUT_DIR/xdm-firefox-extension-9.0.0.xpi"
else
    echo "No Mozilla API keys provided, creating unsigned XPI..."
    zip -r "$OUT_DIR/xdm-firefox-extension-9.0.0.xpi" .
fi
cd ../../..
...
tar -czvf "$OUT_DIR/xdm-linux-x64-9.0.0.tar.gz" *
cd ../..

echo "Creating RPM Package..."
cd app/XDM/XDM.Linux.Installer
rm -rf rpmbuild xdm-9.0.0.tar.gz xdm-9.0.0

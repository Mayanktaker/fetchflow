...
document.addEventListener("click", (e) => {
    ...
    // DOM click: no pre-captured data, send intent for background to re-fetch
...
function handleBlobDownloadIntent(data) {
    ...
    if (base64) {
        // Page context captured the blob data — send directly to background
        sendBlobToBackground(blobUrl, filename || deriveFilename(blobUrl, mime), mime, size, base64);
    } else {
        // Fallback: no data captured, background must re-fetch via scripting.executeScript

...
"use strict";
// Shared junk-URL filter (module twin: ../firefox-amo/noise-filter.js keeps the
// same list for classic scripts; core mirrors it in NetworkHelper.cs).
export const NOISE_URL_SUBSTRINGS = [
...
export function matchesFileExtInUrl(url, file, fileExts) {
    try {
        const full = (url + " " + (file || "")).toUpperCase();
        if ((fileExts || []).find(ext => full.indexOf("." + ext + ".") >= 0)) {
            return true;
        }
        const u = new URL(url);
        for (const [, value] of u.searchParams) {
            const v = ("" + value).toUpperCase();
            if ((fileExts || []).find(ext => v.endsWith("." + ext))) {
                return true;
            }
        }
    } catch { }

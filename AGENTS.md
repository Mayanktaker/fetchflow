...
| **Version Source of Truth** | `app/XDM/XDM.Linux.Installer/version.env` | Currently `9.1.14` (synced with `AppInfo.cs` and manifests) |
...
2. **Version Bump:** For new releases, bump `version.env` first (e.g. `9.1.13` → `9.1.14`), then sync `AppInfo.cs` and `manifest.json` files.

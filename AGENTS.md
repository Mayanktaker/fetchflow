...
| Windows UI | WPF `XDM.Wpf.UI` at GTK feature parity since 9.1.15.6: themes via `Utils/WpfThemeManager.cs` (live swap), Checksum + Import/Export dialogs, theme/scheme settings. Scheme data single-sourced in `XDM.Core/UI/ColorSchemeTable.cs` (both UIs consume it — keep hex tokens in sync there only) |
...
| Version | `app/XDM/XDM.Linux.Installer/version.env` — currently `9.1.15.6` (sync `AppInfo.cs` + both `manifest.json` + WPF `<AssemblyVersion>` + `.iss` `AppVersion` default) |

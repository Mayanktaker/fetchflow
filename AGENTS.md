...
| **Version Source of Truth** | `app/XDM/XDM.Linux.Installer/version.env` | Currently `9.1.8` (synced with `AppInfo.cs` and manifests) |
...
| `dotnet app/XDM/XDM.Tests/bin/Release/net8.0/XDM.Tests.dll` | Run 41-test automated suite (MSTest, YDL parser, JSON, Glade wiring, Language, Branding, GTK smoke) |
| `scripts/run-gtk-smoke.sh` | Run headless GTK builder/autoconnect smoke tests under virtual Xvfb display |
| `dotnet build -c Release -f net4.7.2 app/XDM/XDM.Wpf.UI/XDM.Wpf.UI.csproj` | Compile Windows WPF application binary (`xdm-app.exe`) |
